...
describe('fetchLoginConfig', () => {
    it('passes clientId as a query param and normalizes the body', async () => {
        ...
        const cfg = await fetchLoginConfig(makeHttp(get), 'client-123')
    ...
    it('omits clientId param when none is given', async () => {
        const get = vi.fn().mockResolvedValue({
    ...
    it('returns null on any error (graceful fallback)', async () => {
        ...
        expect(await fetchLoginConfig(makeHttp(get), 'c1')).toBeNull()
    })
